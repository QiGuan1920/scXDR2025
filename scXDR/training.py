# -*- coding: utf-8 -*-
"""
Created on Thu Nov 14 14:36:57 2024
...
def train_model(S1, S2, epochs):
    ...
    # 设置结果的主目录路径
    results_dir = "C:/Users/78760/Desktop/scADR/试运行/结果"
    S1_S2_dir = f"{results_dir}/{S1}_{S2}"
    # 创建 S1_S2 文件夹以及其下的 tables 和 models 子文件夹
    ...
    for epoch in range(epochs):
        ...
        all_pred_true_dfs.append(pred_true_df)  ### 列表格式
    ...
    # 保存开始时间、结束时间、运行时间（秒和化整后格式）到表格文件夹
    ...
    torch.save(optimizer.state_dict(), f"{models_subdir}/optimizer_{S1}_{S2}.pth")  # 可选：保存优化器状态
